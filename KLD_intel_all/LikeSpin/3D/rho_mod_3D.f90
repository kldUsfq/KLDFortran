...
CONTAINS
     ...
     DOUBLE PRECISION FUNCTION f_cutoff(rho)

     IMPLICIT NONE

     DOUBLE PRECISION, INTENT(IN) :: rho
     double precision, parameter :: rho_cut = 0.001d0

            f_cutoff = 0.5d0*(1.D0+DERF(0.5D0*DLOG(rho/rho_cut)))

     END FUNCTION f_cutoff

     DOUBLE PRECISION FUNCTION KLD1_FUNC(x1,y1,z1,x2,y2,z2,NMO,NEL,NOCC,NA,NPO,X0,Y0,Z0,IX,NX,ALPHA,C)
     ...
     fco = f_cutoff(rho(x2,y2,z2,NMO,NOCC,NPO,X0,Y0,Z0,IX,NX,ALPHA,C)) 
...
&                              NMO2,NOCC2,NEL2,NA2,NPO2,X02,Y02,Z02,IX2,NX2,ALPHA2,C2)
     ...
     fco = f_cutoff(rho(x2,y2,z2,NMO1,NOCC1,NPO1,X01,Y01,Z01,IX1,NX1,ALPHA1,C1))
