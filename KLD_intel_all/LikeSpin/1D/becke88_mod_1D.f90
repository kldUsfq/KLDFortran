...
! find out MY process ID, and how many processes were started.
     ...
     if(my_id==0) then
       ...
       if (INX==0.AND.INY==0) ALLOCATE(total_int(INZ))
       if (INX==0.AND.INZ==0) ALLOCATE(total_int(INY))
       if (INY==0.AND.INZ==0) ALLOCATE(total_int(INX))
     end if
...
! end radial loop
     ...
     IF (INX==0.AND.INY==0) ALLOCATE(partial_int(INZ))
     IF (INX==0.AND.INZ==0) ALLOCATE(partial_int(INY))
     IF (INY==0.AND.INZ==0) ALLOCATE(partial_int(INX))
...
! the WFN information is sent from the root to all others process 
     ...
     IF(INX==0.AND.INY==0) THEN
       IN1=INZ
     END IF
     IF(INX==0.AND.INZ==0) THEN
       IN1=INY
     END IF
     IF(INY==0.AND.INZ==0) THEN
       IN1=INX
     END IF
     DO IJK = (my_id+1),npoints,num_procs
        k_procs = k_procs + 1
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(X1,Y1,Z1,III,KLD)
        DO III = 1,IN1
              IF (INX==0) THEN
                 X1 = XX0 
              ELSE
...
&                wfn(2)%X0,wfn(2)%Y0,wfn(2)%Z0,wfn(2)%IX,wfn(2)%NX,wfn(2)%ALPHA,wfn(2)%C,rho_cut)
              ...
              KLD = 0.0 
              END IF
              partial_int(III) = partial_int(III) + w_becke(IJK)*KLD
        END DO
!$OMP END PARALLEL DO
...
! total int appears in the root process
     ...
     if (INX==0.AND.INY==0) call MPI_REDUCE(partial_int,total_int,INZ,MPI_DOUBLE_PRECISION,MPI_SUM,root_process,MPI_COMM_WORLD,ierr)
     if (INX==0.AND.INZ==0) call MPI_REDUCE(partial_int,total_int,INY,MPI_DOUBLE_PRECISION,MPI_SUM,root_process,MPI_COMM_WORLD,ierr)
     if (INY==0.AND.INZ==0) call MPI_REDUCE(partial_int,total_int,INX,MPI_DOUBLE_PRECISION,MPI_SUM,root_process,MPI_COMM_WORLD,ierr)
...
!!!!!   CALL cube_output(total_int)
        ...
        DO III = 1,IN1
              ...
              IF (INY==0.AND.INZ==0) THEN
                  X1 = XX0 + float(III-1)*IXA 
              END IF
              write(6,'(2(F12.6,4X))') X1,total_int(III) 
        END DO
